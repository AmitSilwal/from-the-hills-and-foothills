---
type: research_hub
topic: duar_war
period: 1864_1865
region: dooars
---

# Duar War Research Hub

This page connects all research material related to the [[1864_Duar_War]].

---

## Main Event

- [[1864_Duar_War]]

---

## Pre-War Context

Events that led to the conflict:

- [[1772_British_Bhutan_Intercourse]]
- [[1792_Bijni_Dispute]]
- [[1815_KishenKant_Bose_Mission]]
- [[Ashley_Eden_Mission_1864]]

---

## Military Campaign

Geographical flow of the campaign:

- [[1864_Duar_War_Campaign_Map]]

Important frontier locations:

- [[Maynaguri]]
- [[Dhamonee]]
- [[Dalimkote]]
- [[Chamurchi]]
- [[Buxa]]
- [[Balia]]
- [[Dewangiri]]
- [[Koomrekatta]]

---

## Commanders and Leadership

- [[1864_Duar_War_Commanders]]

---

## Frontier System Concepts

- [[Dooars_Passes]]
- [[Bhutan_Administration_of_Dooars]]
- [[Eighteen_Dooars_(Frontier_Passes)]]

---

## Outcome

- [[1865_Treaty_of_Sinchula]]

---

## Sources

- [[Bhutan_and_the_Dooar_War]]

---

## Related Geography

- [[Dooars]]
- [[Teesta_River]]
- [[Raidak_River]]
- [[Sankosh_River]]

## Related Historical Events

```dataview
TABLE year, region
FROM "02_History"
WHERE region = "dooars"
SORT year asc
```
## Places Mentioned in the Campaign

```dataview
LIST
FROM "06_Places"
WHERE region = "dooars"
SORT file.name
```
## Sources

```dataview
LIST
FROM "08_Sources"
WHERE contains(file.name, "Dooar") OR contains(file.name, "Bhutan")
```
